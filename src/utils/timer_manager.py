    ...
    async def countdown():
        try:
            if seconds > 180:
                ...
                await interaction.followup.send(f"⏰「{boss_name}」在遊戲頻道 {game_channel} 還剩 3 分鐘！")

            await asyncio.sleep(180)
            await interaction.followup.send(f"🔔 {interaction.user.mention} 「{boss_name}」在遊戲頻道 {game_channel} 可能已經重生囉！")
